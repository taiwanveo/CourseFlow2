...
━━━ 版面鐵則（違反 = 輸出廢棄） ━━━
• body 用 flex column 或 grid 分成「圖形演示區」與「文字說明區」兩個獨立的 block
• 兩區在佈局流中各佔 flex/grid 一格 —— 禁止兩者在同一個位置疊排
• 裝飾性背景（光暈、紋理、粒子底層）只能放在 z-index:0 的 ::before/::after 或獨立背景 div，並設 pointer-events:none
• 禁止對主要文字段落使用 position:absolute（標題、步驟列表、說明段落）
• 任何 overflow 不得溢出 viewport
• 【文字字數上限】整個動畫頁面所有可見文字加總 ≤ 30 字，標題 ≤ 8 字，標籤 ≤ 4 字，不得有長句段落
...
${ctx.animationPrompt ? `使用者指定動畫方向：\n${ctx.animationPrompt}\n` : ""}請根據上方概念，設計一個「視覺演示動畫」——找一個具體的動態視覺隱喻（流程箭頭、數字計數、圖形對比、路徑自繪等）來呈現這個概念。動畫裡的文字總字數 ≤ 30 字，不得有長段落。輸出自包含 HTML，把 <theme-css> 原封不動放在 <head><style> 頂部。`;
}
  ...
  try {
    const res = await openai.chat.completions.create({
      ...
      messages: [
        ...
        {
          role: "user",
          content: buildAnimationUserPrompt({
    ...
    animationHtml = res.choices[0]?.message?.content?.trim() ?? "";
    if (!animationHtml.includes("<html")) {
      return NextResponse.json(
        { error: "AI 未能生成有效的 HTML 動畫，請重試" },
        { status: 500 },
      );
    }
  } catch (e) {
