import { preloadThemePreviewUrl } from "@/lib/theme-preview-cache";
...
export function themeGalleryImageUrl(themeId: string): string {
  return THEME_GALLERY[themeId]?.imageUrl ?? `${API_BASE}/${themeId}`;
}
...
export function preloadThemeGalleryImages(themeIds: string[]): void {
  ...
  for (const themeId of themeIds) {
    ...
    seen.add(url);
    preloadThemePreviewUrl(url);
  }
}

/** 進入 Craft 頁時預載全部主題預覽（約 2MB，換主題零等待） */
export function preloadAllThemeGalleryImages(): void {
  preloadThemeGalleryImages(Object.keys(THEME_GALLERY));
}
