...
/** 口播字數：短句偏輕快、長句偏沉穩 */
const NARRATION_SHORT_MAX_CHARS = 28;
const NARRATION_LONG_MIN_CHARS = 48;
...
/** 依口播長度建議進場動畫（短句 drop-in、長句 rise-soft） */
export function pickEnterByNarrationLength(narration: string): string | null {
  const len = narration.trim().length;
  if (len === 0) return null;
  if (len <= NARRATION_SHORT_MAX_CHARS) return "drop-in";
  if (len >= NARRATION_LONG_MIN_CHARS) return "rise-soft";
  return null;
}
...
export function pickContentAwareStepMotion(
  ...
  const lenEnter = pickEnterByNarrationLength(narration);
  if (lenEnter) {
    return { enterAnimationId: lenEnter, transitionId: "crossfade" };
  }
  return {
