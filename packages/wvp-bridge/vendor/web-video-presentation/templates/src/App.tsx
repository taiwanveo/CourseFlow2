...
import "./styles/stage-text-fit.css";
...
import { CHAPTER_TRANSITIONS } from "./registry/chapter-transitions";
import { StepTransitionFrame } from "./components/StepTransitionFrame";
...
export default function App() {
  ...
  // 各主題／版型：步驟切換後自適應縮字，避免溢出舞台或字幕帶
  useEffect(() => {
    scheduleStageTextFit();
  }, [stepper.cursor.chapter, stepper.cursor.step, mode, autoStarted]);
  ...
  const recording = isRecordingMode() || exportMode;
  ...
  const [stepAudioEl, setStepAudioEl] = useState<HTMLAudioElement | null>(null);
  ...
  const onStageAdvance = pauseCtl.paused
    ? () => {}
    : () => {
        unlockAudioPlayback();
        stepper.next();
  ...
  return (
    <>
      <Stage onAdvance={onStageAdvance}>
        <StepTransitionFrame
          stepKey={stepper.cursor.chapter}
          transitionId={
            stepper.cursor.chapter > 0
              ? (CHAPTER_TRANSITIONS[stepper.cursor.chapter - 1] ?? "crossfade")
              : "none"
          }
        >
          <div key={stepper.cursor.step} className="scene">
      ...
      <SubtitleBar
        ...
        audioElement={stepAudioEl}
        sentenceSync={mode !== "manual" && stepText.trim().length > 0}
      />
      <TopMenu
        mode={mode}
        recording={recording}
